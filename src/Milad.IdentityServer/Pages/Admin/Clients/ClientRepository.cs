...
public class ClientRepository
{
    ...
    public async Task UpdateAsync(ClientModel model)
    {
        ...
        if (client.ClientName != model.Name) client.ClientName = model.Name?.Trim();
        ...
        if (scopesToRemove.Any()) client.AllowedScopes.RemoveAll(x => scopesToRemove.Contains(x.Scope));
        if (scopesToAdd.Any())
